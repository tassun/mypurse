				...
				<c:forEach var="fsElement" items="${fsTULJournal.elements()}" varStatus="records">
					<c:if test="${!fsGlobal.nextChapter(records.count)}">
						<tr>
							...
							<td class="text-center"><c:out value="${fsElement.curtime}"></c:out></td>
							<td><c:out value="${fsElement.progid}"></c:out></td>
							<td style="vertical-align: middle;"><c:out value="${fsElement.action}"></c:out></td>
							<td style="vertical-align: middle;"><c:out value="${fsElement.remark}"></c:out></td>
						</tr>
					</c:if>
				</c:forEach>
			</c:when>
