...
<jsp:useBean id="fsTULJournal" scope="request" class="com.fs.bean.TULBean"/>
<jsp:setProperty name="fsTULJournal" property="*"/>
...
<%
session.setAttribute("fsTULJournal",fsTULJournal);
...
String fs_forwarder = "/page_user/page_user_journal_d.jsp";
System.out.println("page_user_journal_c.jsp > "+fs_forwarder);
int fs_action = fsGlobal.parseAction();
try { 
	fsTULJournal.obtain(session,request);
	if(fsTULJournal.getUserid()==null || fsTULJournal.getUserid().trim().length()<=0) {
		fsTULJournal.setUserid(fsAccessor.getFsUser());
	}
	fsTULJournal.transport(fsGlobal);
}catch(Exception ex) { 
...
fsTULJournal.obtain(session,request);
if(fsIsJSONData) {
	out.print(fsTULJournal.toJSONData("rows"));
	return;
}
if(fsIsJSON) {
	out.print(fsTULJournal.toJSON());
	return;
}
if(fsIsXMLData) {
	out.print("<?xml version=\"1.0\" encoding=\""+GlobalVariable.getEncoding()+"\"?>");
	out.print(fsTULJournal.toXMLDatas());
	return;
}
if(fsIsXML) {
	out.print("<?xml version=\"1.0\" encoding=\""+GlobalVariable.getEncoding()+"\"?>");
	out.print(fsTULJournal.toXML());
	return;
}
