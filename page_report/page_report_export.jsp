...
String fs_forwarder = "/page_report/page_report_export_excel.jsp";
String fs_reporttype = request.getParameter("reporttype");
if("pdf".equalsIgnoreCase(fs_reporttype)) fs_forwarder = "/page_report/page_report_export_pdf.jsp";
System.out.println("page_report_export.jsp > "+fs_forwarder);
try { 
	...
	if(fsTRXDReportBean.size()>0) {
		GlobalBean global = (GlobalBean)fsGlobal.clone();
		global.setFsAction(GlobalBean.RETRIEVE_MODE);
		fsTUserReportBean.obtain(fsGlobal);
		fsTUserReportBean.obtain(session,request);
		fsTUserReportBean.setUserid(fsTRXDReportBean.getUserid());
		fsTUserReportBean.transport(global);
	}
}catch(Exception ex) { 
