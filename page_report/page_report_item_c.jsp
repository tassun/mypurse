...
<jsp:setProperty name="fsTRXDItem" property="*"/>
...
String fs_forwarder = "/page_report/page_report_item_d.jsp";
System.out.println("page_report_item_c.jsp > "+fs_forwarder);
int fs_action = fsGlobal.parseAction();
try { 
	fsGlobal.setFsAction(GlobalBean.COLLECT_MODE);
	fsTRXDItem.obtain(session,request);
	if(fsTRXDItem.getUserid()==null || fsTRXDItem.getUserid().trim().length()<=0) {
		fsTRXDItem.setUserid(fsAccessor.getFsUser());
	}
	fsTRXDItem.transport(fsGlobal);
}catch(Exception ex) { 
...
fsTRXDItem.obtain(session,request);
if(fsIsJSONData) {
	out.print(fsTRXDItem.toJSONData("rows"));
	return;
}
if(fsIsJSON) {
	out.print(fsTRXDItem.toJSON());
	return;
}
if(fsIsXMLData) {
	out.print("<?xml version=\"1.0\" encoding=\""+GlobalVariable.getEncoding()+"\"?>");
	out.print(fsTRXDItem.toXMLDatas());
	return;
}
if(fsIsXML) {
	out.print("<?xml version=\"1.0\" encoding=\""+GlobalVariable.getEncoding()+"\"?>");
	out.print(fsTRXDItem.toXML());
	return;
}
