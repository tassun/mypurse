...
try {
	...
	fs_writer.addHeader("cdrefdate",fsLabel.getText("cdrefdate_column","Date")).setAlignment("center");
	...
	fs_writer.addonHeader(new TbField(0,fsLabel.getText("asat_column","As At")+" : "+fsTRXMDailyBean.getFromdate()+" - "+fsTRXMDailyBean.getTodate()));
	fsTRXMDailyBean.getReformAdapter().setRemoveCommaSeparator(true);
	if(fsTRXMDailyBean.size()>0) {
		java.math.BigDecimal fs_sum_recamt = java.math.BigDecimal.ZERO;
		java.math.BigDecimal fs_sum_payamt = java.math.BigDecimal.ZERO;
		java.util.Enumeration ens = fsTRXMDailyBean.elements();
		if(ens!=null) {
		...
		TbField f_sum = new TbField(1,fs_sum_recamt.toString(),true);
		...
		fs_writer.addonRowFooter(new TbField(0,fsLabel.getText("total_column","Total Amount"),true),f_sum,f_pay,f_amt);
		out.clear();
		fs_writer.execute(request,response,"report_daily",fsTRXMDailyBean.iteratorElements());		
	} else {
		out.clear();
		fs_writer.process(request,response,"report_daily",fsLabel.getText("recordnotfound","Records not found"));		
	}
	return;
} catch(Throwable ex) {
