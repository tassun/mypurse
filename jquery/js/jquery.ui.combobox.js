...
$.widget('ui.combobox', {
	...
	init: function() {
		...
		if(options.button) {
			...
			var $atd = $("<td></td>").append(this.arrowElem);
...
$.fn.lookup = function(settings) {
	...
	var lookupDefaults = {
			...
			afterLookup : function(data) { 
				that.val(data.cell[0]); 
				try { $("#"+that.attr("id")+"desc").html(data.cell[1]); }catch(ex) { }
			},	
	...
	if(p.defaultColumnModel) {
		var autoHeight = p.dialogHeight==lookupDefaults.dialogHeight?270:p.dialogHeight;
		p = $.extend({
				columnModel : [
	...
	if(p.defaultSearchItems) {
		var autoHeight = p.dialogHeight==lookupDefaults.dialogHeight?270:(p.dialogHeight==270?300:p.dialogHeight);
		p = $.extend({
				searchItems : [
